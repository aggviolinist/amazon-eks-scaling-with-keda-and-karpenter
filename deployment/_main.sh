...
if [[ "$user_input" == *"$Entry"* ]]; then
    ...
    if [[ "$user_input" == $CLUSTER ]]; then
        ...
        chmod u+x "$SCRIPT_DIR/cluster/createCluster.sh"
        ...
        echo $CLUSTER_NAME "|"  $KARPENTER_VERSION  "|" $AWS_REGION  "|" $ACCOUNT_ID  "|" $TEMPOUT
        chmod u+x "$SCRIPT_DIR/karpenter/createkarpenter.sh"
        ...
        chmod u+x "$SCRIPT_DIR/keda/createkeda.sh"
        "$SCRIPT_DIR/keda/createkeda.sh"

        echo "${GREEN}Deploy Demo components DynamoDB and SQS!!"
        chmod u+x "$SCRIPT_DIR/services/awsService.sh"
